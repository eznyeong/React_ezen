...
import { Link } from 'react-router-dom'; // 페이지 이동 컴포넌트 호출
...
function Header() {
    ...
    //페이지들이 초기실행 될 때마다 스크롤 처리
    //아래 방법으로 적으면 렌더링하면서 계속 DOM을 탐색해서 React 특성상 성능에 문제가 발생
    /*
        useEffect(()=>{
    ...
    */
    ...
    //성능 개선을 위해서 스크롤 이벤트에 Timer를 더서, 이벤트가 발생된 후 Timer 초기화해서 성능 지연 방지
    useEffect(()=>{
    ...
    const handleScroll = () => {
        ...
        if(windowTop > 50){
            //classList : 문서객체의 클래스관련을 관리하는 바닐라 자바스크립트의 속성
    ...
    return (
        <HeaderBlock ref={header}>
            <h1>
                <Link to="/">
                    <img src="https://www.tving.com/img/tving-new-logo-pc.svg" alt="티빙로고" />
                </Link>
            </h1>
            <ul>
