...
function MainList({ mains }) {
    //swiper요소를 담든 state선언
    const [swiper, setSwiper] = useState(null); //초기 값은 비워두기

    //play/pause관련 state선언
    const [cnt, setCnt] = useState(false); //play상태를 false, pause상태를 true로 처리

    //특정 객체 선택 - ref로 처리
    const btnPause = useRef();
    const btnPlay = useRef();

    //초기 실행 시 메인이미지 스와이퍼를 담을 예정
    useEffect(() => {
        //ref를 통해서 선택해도 선택이 잘 안됨 => 아래와 같이 자바스크립트로 직업적으로 선택
