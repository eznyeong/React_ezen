...
//2. 유틸로부터 함수 호출 - 전체데이터관려나, 각각데이터관련 모두 호출
import { createPromiseThunk, createPromiseThunkById, reducerUtils, handleAsyncActions, handleAsyncActionsById } from '../libs/asyncUtils';

//3. 액션타입명 선정 - 전체데이터 3개, 각각 데이터 3개
// 여러개 조회하기
...
export const getTvingBest = createPromiseThunkById(GET_TVINGBEST, tvingBestAPI.getTvingBestId);
...
//6. 리듀서 함수 선언
export default function tvingbests(state=initalState, action){
    switch (action.type){
        case GET_TVINGBESTS:
