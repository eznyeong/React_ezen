// 방탄소년단 데이터를 가짜 API로 만들고 호출할 문서

// n 밀리세컨드 동안 기다리는 프로미스 함수 제작
// 데이터가 별로 없어서 로딩 중 상황을 볼 수가 없어서 일부러 기다리는 시간 제작
...
export const getBts = async () => {
    await sleep(500); // 0.5초를 로딩하고 - 실제 데이터를 가져올 때 작성 X
    return bts; // bts 데이터 반환
}

// 방탄소년단 각각 데이터 목록을 가져오는 비동기 함수 선언
export const getBtById = async id => {
    await sleep(500); // 0.5초의 로딩시간을 갖고
    return bts.find(bt => bt.id === id); // id값으로 한명한명 데이터를 각각 호출할 예정
}
