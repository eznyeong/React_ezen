...
export const getBts = () => async dispatch => {
    dispatch({ type: GET_BTS }); //로딩 시작
    try{
        const bts = await btsAPI.getBts(); //로딩 중
        dispatch({ type: GET_BTS_SUCCESS, bts }); //성공 확인
    }catch(e){
        dispatch({ type: GET_BTS_ERROR, error: e }); //에러 처리
    }
}
export const getBt = id => async dispatch => {
    dispatch({ type: GET_BT }); //로딩 시작
    try{
        const bt = await btsAPI.getBtById(id); //로딩 중
        dispatch({ type: GET_BT_SUCCESS, bt }); //성공 확인
    }catch(e){
        dispatch({ type: GET_BT_ERROR, error: e }); //에러 처리
    }
}
...
const initialState = {
    bts: { //전체 목록 데이터
        loading: false, //로딩 중일 때는 true로 변경
        ...
        case GET_BT_SUCCESS:
            return {
                ...
                bt: {
                    ...
                    data: action.bt, //각각 멤버별로 데이터를 반환
                    error: null //에러는 없음
                }
            }
        case GET_BT_ERROR:
